...
exports.signature = (req, res) => {
  ...
  //给新名字加上原来的后缀
  let newName = req.files[0].originalname;
  fs.renameSync("./public/upload/" + oldName, "./public/signature/" + newName); //改图片的名字注意此处一定是一个路径，而不是只有文件名
  const filePath = "http://127.0.0.1:12134/signature/" + newName;
  fs.access(filePath, fs.constants.F_OK, (err) => {
    if (err) {
      console.log(err);
      // console.error(`文件不存在: ${filePath}`);
  ...
  res.send({
    err: 0,
    url: "http://127.0.0.1:12134/signature/" + newName,
  });
};
